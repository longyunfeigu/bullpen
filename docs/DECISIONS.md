...
| [ADR-0043](adr/ADR-0043-unsigned-public-beta-release.md) | Accepted | 零成本 unsigned public beta：三平台 GitHub prerelease + checksums/SBOM；签名 Stable 保持外部阻断且策略禁止冒充 | 2026-07-21 | M12-01..07、§12.2、§14 M12、§16、§18.2/18.3 |
...
| [ADR-0044](adr/ADR-0044-session-orchestration.md) | Accepted | 会话编排（agent 指挥 agent）：六个 terminal.* gateway 工具（send 文本过分类器动态定级、R4 前置拒绝不变）+ per-terminal token 身份（内存态不落盘）+ unix socket 外部门（claude/codex 当司机，验票→翻译进同一 executeCall）+ 长在 rail/房间的指挥 UI（家谱缩进 44px 子行、编队区双列瓦片、导播位叠加）；两层封顶、两档暂停、敲字即接管、审批单记录多入口、read/wait 只记元数据不落输出、总开关熔断；**2026-07-22 修订**：read 与 TUI-send 降 R0 免审（用户裁定观察/内容注入不再弹卡；裸 shell R2 下限、控制键 R2、create R2、kill R4、预算/接管/暂停全不变；read 进 Ask catalog） | 2026-07-21 | ORCH-001..012（M13 立项，M12 先行）、ADR-0002/0006/0015/0017/0021/0023/0030、PIVOT-013/014、fusion+v3-director-stage mockup 验收；外部参考 alchaincyf/fanbox docs/12 |
